...
public class ChessMatch {
	...
	public ChessPiece performChessMove( ChessPosition sourcePosition, ChessPosition targetPosition ) {
		...
		// Verificando se o jogador se colocou em check mate.
		...
		// Verificando se o oponente ficou em check mate.
		check = ( testCheck( opponent( currentPlayer ) ) ) ? true : false;
		
		// Testando se a jogada deixou o oponente em checkmate.
		...
		// Testando se a peça que se moveu foi um peão e se ele se moveu duas casas, pela primeira vez.
		// Se isso ocorreu, esse peão fica marcado como en passant.
	...
	public ChessPiece replacePromotedPiece( String type ) {
		...
		if ( ( !type.equals( "B" ) ) && ( !type.equals( "N" ) ) && ( !type.equals( "R" ) ) && ( !type.equals( "Q" ) ) ) {
			throw new IllegalStateException( "Tipo inválido para promoção!" );
		}
	...
	private Piece makeMove( Position source, Position target ) {
		...
		if ( p instanceof Peao ) {
			// Se o peão andou na diagonal e não fez nenhuma captura, então é um en passant.
	...
	private void undoMove( Position source, Position target, Piece capturedPiece ) {
		...
		if ( p instanceof Peao ) {
			if ( ( source.getColumn() != target.getColumn() ) && ( capturedPiece == enPassantVulnerable ) ) {
				...
				board.placePiece( peao, pawnPosition );
			}
		}
	}
		...
		for ( Piece p : list ) {
			...
			for ( int i = 0; i < board.getRows(); i++ ) {
				for ( int j = 0; j < board.getColumns(); j++ ) {
					...
					// Movendo o rei para testar se após mover o mesmo ainda continua em check.
					if ( mat[ i ][ j ] ) {
